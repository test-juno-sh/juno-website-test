...
Yes, absolutely! Your PC will need to be switched on and accessible to your iPad at all times though. There are a couple of ways of connecting to it: if your iPad is on the same local network, you can just use your PC's IP address. If you would like to connect over the internet, you would need to configure your PC as a public server. Please, check [Jupyter documentation](http://jupyter-notebook.readthedocs.io/en/stable/public_server.html){:target="_blank"} for further details.
